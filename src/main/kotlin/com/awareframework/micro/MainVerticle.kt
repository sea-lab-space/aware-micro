...
class MainVerticle : AbstractVerticle() {
  ...
  override fun start(startPromise: Promise<Void>) {
    ...
    val serverOptions =
            HttpServerOptions()
                    ...
                    .setMaxChunkSize(1024 * 1024 * 50)
                    .setMaxInitialLineLength(1024 * 1024 * 50)
                    .setMaxHeaderSize(1024 * 1024 * 50)
                    .setMaxFormAttributeSize(-1)
    val pebbleEngine =
            PebbleTemplateEngine.create(vertx, PebbleEngine.Builder().cacheActive(false).build())
    ...
    router.route().handler {
      logger.info {
        "Processing ${it.request().scheme()} ${it.request().method()} : ${it.request().path()} with the following data ${it.request().params().toList()}"
      }
      it.next()
    }
    ...
    val configRetrieverOptions =
            ConfigRetrieverOptions().setScanPeriod(5000).addStore(configJsonFile)
    ...
    configReader.getConfig { config ->
      if (config.succeeded() && config.result().containsKey("server")) {
        ...
        // HttpServerOptions.host is the host to listen on. So using |server_host|, not
        // |external_server_host| here.
        // See also:
        // https://vertx.io/docs/4.3.3/apidocs/io/vertx/core/net/NetServerOptions.html#DEFAULT_HOST
        serverOptions.host = serverConfig.getString("server_host")

        /**
         * Uncomment this route for debug sake Handles incoming HTTP requests, extracts and
         * processes form-encoded data, and logs key request details before passing it to the next
         * handler.
         *
        ...
        router.route().handler { routingContext ->
          ...
          // Find the position of `&data=` to ensure that everything after `data` is not split by
          // `split("&")`
          val dataIndex = body.indexOf("&data=")
          val devicePart: String
          val dataPart: String

          if (dataIndex != -1) {
            devicePart = body.substring(0, dataIndex) // Extract the part before `data=`
            dataPart = body.substring(dataIndex + 6) // Extract the JSON after `data=`
          } else {
            devicePart = body
            dataPart = ""
          }

          // Parse the device_id section
          val deviceParams = devicePart.split("&")
          ...
          // Store data directly without URL decoding
          paramMap["data"] = dataPart
        ...
        fun parseFormEncodedBody(body: String): Map<String, String> {
          ...
          for (param in deviceParams) {
            ...
            if (parts.size == 2) {
              val key = parts[0]
              val value = URLDecoder.decode(parts[1], StandardCharsets.UTF_8.name()) // Perform URL decoding
              paramMap[key] = value
            }
          }
          ...
          if (validRoute(
            ...
            vertx.fileSystem().open(
              ...
              if (write.succeeded()) {
                ...
                client.get(443, "qrcode.tec-it.com", "/API/QRCode?size=small&data=$serverURL")
                        .`as`(BodyCodec.pipe(asyncQrcode, true))
                        .send { request ->
                          if (request.succeeded()) {
                            pebbleEngine.render(
                                    JsonObject().put("studyURL", serverURL),
         ...
         * - when joining the study, returns the JSON with all the settings from the study. Can be
         * called from apps using Aware.joinStudy(URL) or client's QRCode scanner
          ...
          if (validRoute(
            ...
            if (route.request().getFormAttribute("study_check") == "1") {
              ...
              status.put(
                      "config",
                      "[]"
              ) // NOTE: if we send the configuration, it will keep reapplying the settings on
              // legacy clients. Sending empty JsonArray (i.e., no changes)
        ...
        /**
         * Legacy: this will be hit by legacy client to retrieve the study information. It retuns
         * JsonObject with (defined also in aware-config.json on AWARE Micro): { "study_key" :
         * "studyKey", "study_number" : 1, "study_name" : "AWARE Micro demo study",
         * "study_description" : "This is a demo study to test AWARE Micro", "researcher_first" :
         * "First Name", "researcher_last" : "Last Name", "researcher_contact" : "your@email.com" }
         */
        router.route(HttpMethod.GET, "/index.php/webservice/client_get_study_info/:studyKey")
                .handler { route ->
        ...
        router.route(HttpMethod.POST, "/index.php/:studyNumber/:studyKey/:table/:operation")
                .handler { route ->
                  ...
                  if (validRoute(
                    ...
                    when (route.request().getParam("operation")) {
                      "create_table" -> {
                        // Commented the following line as we merged with insert. Only here so that
                        // legacy client thinks all is ok
                      ...
                      "insert" -> {
                        eventBus.publish(
                                ...
                                JsonObject()
                                        ...
                                        .put(
                                                "device_id",
                                                parsedParams["device_id"]
                                        ) // Extracted from the parsed body
                                        .put(
                                                "data",
                                                parsedParams["data"]
                                        ) // Extracted from the parsed body
                      ...
                      "query" -> {
                        val requestData =
                                JsonObject()
                                        ...
                                        .put(
                                                "start",
                                                route.request().getFormAttribute("start").toDouble()
                                        )
                                        .put(
                                                "end",
                                                route.request().getFormAttribute("end").toDouble()
                                        )
        ...
        // Use SSL
        if (serverConfig.getString("path_fullchain_pem").isNotEmpty() &&
                        serverConfig.getString("path_key_pem").isNotEmpty()
        ) {
          serverOptions.pemTrustOptions =
                  PemTrustOptions().addCertPath(serverConfig.getString("path_fullchain_pem"))
        ...
        httpServer =
                vertx.createHttpServer(serverOptions).requestHandler(router).listen(
                                serverConfig.getInteger("server_port")
                        ) { server ->
                  if (server.succeeded()) {
                    ...
                    logger.info {
                      "AWARE Micro API at ${getExternalServerHost(serverConfig)}:${getExternalServerPort(serverConfig)}"
                    }
        ...
        configReader.listen { change ->
          ...
          if (newServerConfig.getString("path_fullchain_pem").isNotEmpty() &&
                          newServerConfig.getString("path_key_pem").isNotEmpty()
          ) {
            newServerOptions.pemTrustOptions =
                    PemTrustOptions().addCertPath(newServerConfig.getString("path_fullchain_pem"))

            newServerOptions.pemKeyCertOptions =
                    PemKeyCertOptions()
          ...
          httpServer =
                  vertx.createHttpServer(newServerOptions).requestHandler(router).listen(
                                  newServerConfig.getInteger("server_port")
                          ) { server ->
                    if (server.succeeded()) {
                      ...
                      logger.info {
                        "AWARE Micro API at ${getExternalServerHost(newServerConfig)}:${getExternalServerPort(newServerConfig)}"
                      }
                    } else {
                      logger.error(server.cause()) { "AWARE Micro initialisation failed!" }
                    }
        ...
        val sensors =
                getSensors(
                        "https://raw.githubusercontent.com/denzilferreira/aware-client/master/aware-core/src/main/res/xml/aware_preferences.xml"
                )
        ...
        vertx.fileSystem().writeFile(
                        "./aware-config.json",
                        Buffer.buffer(configFile.encodePrettily())
                ) { result ->
          if (result.succeeded()) {
            logger.info {
              "You can now configure your server by editing the aware-config.json that was automatically created. You can now stop this instance (press Ctrl+C)"
            }
          } else {
  ...
  fun validRoute(studyInfo: JsonObject, studyNumber: Int, studyKey: String): Boolean {
    return studyNumber == studyInfo.getInteger("study_number") &&
            studyKey == studyInfo.getString("study_key")
  }
    ...
    for (i in 1..docRoot.length) {
      ...
      if (child != null) {
        ...
        if (child.attributes.getNamedItem("android:icon") != null)
                sensor.put(
                        "icon",
                        getSensorIcon(child.attributes.getNamedItem("android:icon").nodeValue)
                )
        ...
        for (j in 0..subChildren.length) {
          ...
          if (subChild != null && subChild.nodeName.contains("Preference")) {
            ...
            if (subChild.attributes.getNamedItem("android:key") != null)
                    setting.put(
                            "setting",
                            subChild.attributes.getNamedItem("android:key").nodeValue
                    )
            if (subChild.attributes.getNamedItem("android:title") != null)
                    setting.put(
                            "title",
                            subChild.attributes.getNamedItem("android:title").nodeValue
                    )
            if (subChild.attributes.getNamedItem("android:defaultValue") != null)
                    setting.put(
                            "defaultValue",
                            subChild.attributes.getNamedItem("android:defaultValue").nodeValue
                    )
            if (subChild.attributes.getNamedItem("android:summary") != null &&
                            subChild.attributes.getNamedItem("android:summary").nodeValue != "%s"
            )
                    setting.put(
                            "summary",
                            subChild.attributes.getNamedItem("android:summary").nodeValue
                    )

            if (setting.containsKey("defaultValue")) settings.add(setting)
          }
        }
  ...
  private fun getSensorIcon(drawableId: String): String {
    ...
    val downloadUrl =
            "/denzilferreira/aware-client/raw/master/aware-core/src/main/res/drawable/*.png"
